...
- **⑤ malformed データの扱い**: 構築時 malformed は**起動失敗**（fail-fast）。
  起動後のリロードで malformed になった場合は `error!` ログのみで**旧データ継続**（fail-static）。
  → S3 上のポリシーを壊しても即死はしないが、更新が無言で無視されるためアラート必須（§7）。
...
- `file_inspector_task` がマウント上のポリシーファイルを定期ポーリング（SHA256 差分）→
  変化時に `update_provider_data_task` が `file::PolicySetProvider::update_provider_data` を呼ぶ（§5.1）。
- 更新失敗（パースエラー等）は `error!` ログのみで**旧ポリシー継続**（§4 ⑤）。
