...
#
# Multi-stage build for authzen-pdp (DESIGN.md §11).
...
#   docker buildx build --platform linux/amd64 -t <ecr>/authzen-pdp:0.1.0 --push .
...
COPY --from=builder /build/target/release/authzen-pdp /usr/local/bin/authzen-pdp
